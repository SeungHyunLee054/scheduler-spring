...
public class SchedulerService {
    ...
    public Page<SchedulerResponseDto> getAllSchedulers(String name, LocalDate modifiedAt, Pageable pageable) {
        if (name == null && modifiedAt == null) {
            return schedulerRepository.findAll(pageable)
                    .map(Scheduler::toDto);
        } else if (name != null && modifiedAt == null) {
            return schedulerRepository.findAllByName(name, pageable)
                    .map(Scheduler::toDto);
        } else if (name == null) {
            return schedulerRepository.findAllByModifiedAt(modifiedAt, pageable)
                    .map(Scheduler::toDto);
        } else {
            return schedulerRepository.findAllByNameAndModifiedAt(name, modifiedAt, pageable)
                    .map(Scheduler::toDto);
        }
    }
